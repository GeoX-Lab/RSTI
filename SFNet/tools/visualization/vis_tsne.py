...
def get_best_ckpt_path(ckpt_dir):
    ...
    for fname in os.listdir(ckpt_dir):
        if fname.startswith('best_') and fname.endswith('.pth'):
            ...
            break
    return ckpt_dir+'/'+best_ckpt_name


EXP_ID = {
    0: 'resnet50_20240427_110818',
    1: 'freqnet50_20240427_110854',
    2: 'sfnet_full_20240514_062230',
}[2]


MODEL = EXP_ID.split('_2024')[0]
CONFIG = f'configs/0_RSI_Authentication_v2/{MODEL}.py'
CKPT = get_best_ckpt_path('work_dirs/0_RSI_Authentication_v2/' + MODEL)
WORK_DIR = f'2_Log/0_RSI_Authentication_v2/{EXP_ID}'


def parse_args():
    parser = argparse.ArgumentParser(description='t-SNE visualization')
    parser.add_argument('--config', default=CONFIG, help='tsne config file path')
...
def main():
    ...
    for key, val in results.items():
        ...
        if args.legend:
            ...
            # legend_elements = [Patch(facecolor=color_map[i], label=classes[i]) for i in color_map]
            legend_elements = [
                Line2D([0], [0], marker='o', color='w', label=classes[i],
                       markerfacecolor=color_map[i], markersize=10) for i in color_map]
            legend = ax.legend(handles=legend_elements)

            ax.add_artist(legend)
        plt.savefig(f'{tsne_work_dir}{key}.svg')
        if args.show:
            plt.show()
