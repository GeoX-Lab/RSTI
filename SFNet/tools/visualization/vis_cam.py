...
def init_cam(method, model, target_layers, reshape_transform):
    ...
    cam = GradCAM_Class(model=model, target_layers=target_layers)
...
def main():
    ...
    pred = ['0_real', '1_fake'][nn.functional.softmax(model(data['inputs']), dim=1).argmax()]
    model_name = model.backbone._get_name()
    img_name = os.path.basename(args.img).split('.')[0]
    save_path = f'{args.save_dir}/{img_name}_pred_by_{model_name}_as_{pred}.png'
    show_cam_grad(
        grayscale_cam, src_img, title=args.method, out_path=save_path)
