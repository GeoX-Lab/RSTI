...
from utils import mertics2markdown
...
def parse_args():
    ...
    parser.add_argument('--metrics_log_file', default=None, help='log file name')
    parser.add_argument('--metrics_log_file_all', default=None, help='log file name')
...
def main():
    ...
    # extra visualization and log of results
    exp_id = cfg.get('experiment_id', osp.splitext(osp.basename(args.config))[0])
    markdown_table = mertics2markdown(metrics, exp_id)
    print(markdown_table)
    if args.metrics_log_file:
        with open(args.metrics_log_file, 'w') as file:
            file.write(markdown_table)
    if args.metrics_log_file_all:
        if os.path.exists(args.metrics_log_file_all):
            last_content = markdown_table.split('\n')[-1]
            with open(args.metrics_log_file_all, 'a') as file:
                file.write('\n'+last_content)
        else:
            with open(args.metrics_log_file_all, 'w') as file:
                file.write(markdown_table)
