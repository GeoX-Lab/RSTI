...
def parse_args():
    ...
    parser.add_argument('--result', default='', help='test result json/pkl file')
    parser.add_argument(
...
def save_imgs(result_dir, folder_name, results, dataset, rescale_factor=None):
    ...
    for i, data_sample in enumerate(results):
        ...
        save_name = f'{i:04d}_{name}'
        vis.visualize_cls(
            # img, data_sample, out_file=osp.join(full_dir, name + '.png'))
            img, data_sample, out_file=osp.join(full_dir, save_name))

        gt_cls = dataset.CLASSES[data_sample.gt_label]
        out_cls_dir = f'{result_dir}/{folder_name}_{gt_cls}'
        if not osp.exists(out_cls_dir):
            os.makedirs(out_cls_dir)
        shutil.copyfile(img_pth, out_cls_dir+'/'+save_name)
...
def main():
    ...
    success.reverse()  # 最自信的 正确预测样本排前面
    fail.reverse()   # 最盲目自信的 错误预测样本排前面
