...
class ResLayer(nn.Sequential):
    """ResLayer to build FreqNet style backbone.
...
class FreqNet(BaseBackbone):
    ...
    def __init__(self,
                 ...
                 drop_path_rate=0.0):
        super(FreqNet, self).__init__(init_cfg)
        if depth not in self.arch_settings:
            raise KeyError(f'invalid depth {depth} for FreqNet')
        self.depth = depth
        ...
        self.conv1_freq = nn.Conv2d(3, 64, kernel_size=1, stride=1, bias=True)
        self.relu = nn.ReLU(inplace=True)
        self.realconv1 = conv1x1(64, 64, stride=1)
        self.imagconv1 = conv1x1(64, 64, stride=1)

        self.conv2_freq = nn.Conv2d(64, 64, kernel_size=1, stride=2, bias=True)
        self.realconv2 = conv1x1(64, 64, stride=1)
        self.imagconv2 = conv1x1(64, 64, stride=1)

        self.conv3_freq = nn.Conv2d(256, 256, kernel_size=1, stride=1, bias=True)
        self.realconv3 = conv1x1(256, 256, stride=1)
        self.imagconv3 = conv1x1(256, 256, stride=1)

        self.conv4_freq = nn.Conv2d(256, 256, kernel_size=1, stride=2, bias=True)
    ...
    def init_weights(self):
        super(FreqNet, self).init_weights()
    ...
    def get_layer_depth(self, param_name: str, prefix: str = ''):
        """Get the layer id to set the different learning rates for FreqNet.
