...
def get_samples(
    ...
    # Pre-build file backend to prevent verbose file backend inference.
    cls_dir = root + '/' + cls_prefiex
    backend = backend or get_file_backend(cls_dir, enable_singleton=True)

    if folder_to_idx is not None:
        for folder_name in sorted(list(folder_to_idx.keys())):
            _dir = backend.join_path(root, cls_prefiex, folder_name)
            ...
            for file in sorted(list(files)):
                if is_valid_file(file):
                    path = backend.join_path(cls_prefiex, folder_name, file)
...
class ForgeryDataset(BaseDataset):
    ...
    def _find_samples(self):
        ...
        if self.with_label:
            ...
            samples = []
            for cls in self.prefix_classes:
                cls_dir = self.img_prefix + '/' + cls
                classes, folder_to_idx = find_folders(cls_dir)
                _samples, empty_classes = get_samples(
                    self.img_prefix, cls,
                    folder_to_idx,
